...
def build_diverse_beam(input):
    ...
    _abstract = "\n".join(abstract)
    article = []
    revs = src_line.split(' </s> ')
    for rev in revs:
        article += sent_tokenize(rev)
    
    if dataset == "xsum":
        def compute_rouge(hyp):
            score = all_scorer.score(_abstract, "\n".join(hyp))
            if score["rouge1"].fmeasure + score["rouge2"].fmeasure == 0:
                harmonic = 0
            else:
                harmonic = 2 * score["rouge1"].fmeasure * score["rouge2"].fmeasure / (score["rouge1"].fmeasure + score["rouge2"].fmeasure)
            return (2 * harmonic + score["rouge1"].fmeasure + score["rouge2"].fmeasure) / 4
    else:
        def compute_rouge(hyp):
            score = all_scorer.score(_abstract, "\n".join(hyp))
    ...
    article_untok = []
    revs = src_line_untok.split(' </s> ')
    for rev in revs:
        article_untok += sent_tokenize(rev)
