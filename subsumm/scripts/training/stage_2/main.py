...
from config import amasum_cons_setting, amasum_pros_setting, amasum_verd_setting, rotten_setting
...
def evaluation(args):
    # load data
    if args.config == "cons":
        amasum_cons_setting(args)
    elif args.config == "pros":
        amasum_pros_setting(args)
    elif args.config == "verd":
        amasum_verd_setting(args)
    else:
        rotten_setting(args)
    if args.is_pegasus:
    ...
    '''
    test_set = BrioDataset(os.path.join(args.data_path, args.datatype, 'test'), args.model_type, is_test=True, max_len=512,
    ...
    model.load_state_dict(torch.load(os.path.join(args.cache_path, args.model_pt), map_location=f'cuda:{args.gpuid[0]}'))
    ...
    print(model_name)
    root_dir = "/nfs-data/user19/result/%s"%model_name
    mkdir(root_dir)
    ...
    if args.do_reranking:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, total=len(test_set) // batch_size):
                ...
                samples = batch["data"]
                output = model(batch["src_input_ids"], batch["candidate_ids"], args.normalize, args.score_mode, args.rank_length_penalty, adding=args.adding)
    ...
    if args.do_generation:
        ...
        with open(os.path.join(args.data_path, args.datatype, 'test.source')) as source:
            total_num = len(source.readlines())
        with open(os.path.join(args.data_path, args.datatype, 'test.source')) as source, open(os.path.join(root_dir, "test.hypo"), 'w') as fout:
        ...
        with open(os.path.join(root_dir, "test.hypo")) as fout, open(os.path.join(args.data_path, args.datatype, 'test.target')) as target:
...
def run(rank, args):
    ...
    if is_master:
        id = len(os.listdir(args.cache_path))
        recorder = Recorder(args.proj_name, args.cache_path, args.log)
    # build dataloader
    if args.is_pegasus:
        tok = PegasusTokenizer.from_pretrained(args.model_type)
    else:
    ...
    train_set = BrioDataset(os.path.join(args.data_path, args.datatype, 'train'), args.model_type, max_len=args.max_len, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus, no_overlap=args.no_overlap)
    val_set = BrioDataset(os.path.join(args.data_path, args.datatype, 'valid'), args.model_type, is_test=True, max_len=512, is_sorted=False, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus)
    if is_mp:
    ...
    if len(args.model_pt) > 0:
        old_state_dict = torch.load(os.path.join(args.cache_path, args.model_pt), map_location=f'cuda:{gpuid}')['model']
        del old_state_dict['encoder.version']
        del old_state_dict['decoder.version']
        del old_state_dict['encoder.embed_tokens.weight']
        del old_state_dict['decoder.embed_tokens.weight']
        del old_state_dict['decoder.output_projection.weight']
        state_dict = {}
        for k, v in old_state_dict.items():
            state_dict[f'model.model.{k}'] = v
        model.load_state_dict(state_dict, strict=False)
        old_state_dict.clear()
    if args.cuda:
        if is_mp:
            # Using DDP
    ...
    for epoch in range(args.epoch):
        ...
        for (i, batch) in enumerate(dataloader):
            ...
            if epoch_step % args.report_freq == 0 and step_cnt == 0 and is_master:
                ...
                recorder.print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
                recorder.print("epoch: %d, step: %d, avg loss: %.6f, avg ranking loss: %.6f, avg mle loss: %.6f"
            ...
            if all_step_cnt % args.eval_interval == 0 and all_step_cnt != 0 and step_cnt == 0:
                ...
                if loss < minimum_ranking_loss and is_master:
                    ...
                    recorder.print("best ranking loss - epoch: %d, batch: %d"%(epoch+1, i / args.accumulate_step))
                if is_master:
                ...
                if mle_loss < minimum_mle_loss and is_master:
                    ...
                    recorder.print("best generation loss - epoch: %d, batch: %d"%(epoch+1, i / args.accumulate_step))
                if is_master:
...
if __name__ ==  "__main__":
    ...
    parser.add_argument("--config", default="", type=str, help="config path", choices=['cons', 'pros', 'verd', 'rt'])
